...
# ------------------'----------- #
# Deployment of new Rust ERC-721 #
# -----------------'------------ #
echo ""
echo "------------'----------------------"
echo "Deploying new Rust ERC-721 contract"
echo "-----------'-----------------------"
...
erc721_contract_address_str=$(cat $DEPLOY_CONTRACT_RESULT_FILE | sed -n 2p)
if ! [[ $erc721_contract_address_str == *0x* ]]
then
    # When the program needs activation, the output of the command is slightly different
    erc721_contract_address_str=$(cat $DEPLOY_CONTRACT_RESULT_FILE | sed -n 3p)
fi
erc721_contract_address_array=($erc721_contract_address_str)
erc721_contract_address=$(echo ${erc721_contract_address_array[2]} | sed 's/\x1B\[[0-9;]\{1,\}[A-Za-z]//g')
rm $DEPLOY_CONTRACT_RESULT_FILE

# Final result
echo "ERC-721 contract deployed and activated at address: $erc721_contract_address"
...
cast send --rpc-url $RPC_URL --private-key $PRIVATE_KEY $PROXY_ADMIN_ADDRESS "upgradeAndCall(address,address,bytes)()" $PROXY_CONTRACT_ADDRESS $erc721_contract_address 0x

echo "Proxy $PROXY_CONTRACT_ADDRESS was updated to implementation in $erc721_contract_address"
