...
import { ExternalExtension, RawOptions, StylusExtension } from "../types";
...
export const getExtensionDirsFromExternalExtension = async (
  externalExtension: NonNullable<RawOptions["externalExtension"]>,
) => {
  const extensions = Object.values(STYLUS_EXTENSIONS);
  const filterExtensionDirs = (dirs: string[]) => {
    return dirs.filter(dir => extensions.includes(dir as StylusExtension)).reverse() as StylusExtension[];
  };
  ...
  if (typeof externalExtension === "string") {
    ...
    const externalExtensionDirs = await fs.promises.readdir(
  ...
  const extensionChecks = extensions.map(async extension => {
    ...
    try {
      ...
      if (res.status === 200) return extension as StylusExtension;
      if (res.status === 404) return null;

      throw new Error(
        `${extension.charAt(0).toUpperCase() + extension.slice(1)} extension check failed with status ${res.status}. You can verify it at ${githubUrl}.`,
      );
    } catch (err) {
      console.warn((err as Error).message);

      return extension as StylusExtension;
    }
  });

  const results = await Promise.all(extensionChecks);
  const extensionDirs = results.filter((extension): extension is StylusExtension => extension !== null);
